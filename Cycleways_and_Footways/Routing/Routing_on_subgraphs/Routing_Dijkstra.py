...
def creation_map_cost(result_routing_cost, nodes, gdf_cycleways, gdf_footways, path, lat, lon, mapName):
...
def creation_map_travel_time(result_routing_travel, nodes, gdf_cycleways, gdf_footways, path, lat, lon, mapName):
    """Draw the map with the obtained paths"""
...
def creation_map_total(result_routing_cost, result_routing_travel, nodes, gdf_cycleways, gdf_footways, path, lat, lon, mapName):
    """Draw the map with the obtained paths"""
    l_cost = result_routing_cost[0][1]
    l_travel = result_routing_travel[0][1]
...
def main(args=None):
    ...
    if options.weight == "cost":
        ...
        creation_map_cost(result_routing_cost, nodes, gdf_cycleways, gdf_footways, path,
                           options.lat, options.lon, options.mapName)
        print("Creation of the map : done ")

    elif options.weight == "travel_time":
        """Routing considering as weight the cost"""
        result_routing_travel_time = greeter.routing_algorithm_based_on_cost(options.lat, options.lon, options.dest,
                                                                      graph_projection + "_cost")
        print(
            "Find the best path between your source location and the target location, considering the travel time needed : done")

        """Generation of the map with the obtained paths displayed"""
        creation_map_cost(result_routing_travel_time, nodes, gdf_cycleways, gdf_footways, path,
                          options.lat, options.lon, options.mapName)
        print("Creation of the map : done ")

    elif options.weigth == "both":
        """Routing considering as weight the cost"""
        result_routing_cost = greeter.routing_algorithm_based_on_cost(options.lat, options.lon, options.dest,
                                                                      graph_projection + "_cost")
        print(
            "Find the best path between your source location and the target location, considering the travel time needed and the level of security of the paths used : done")
        ...
        """Generation of the map with the obtained paths displayed"""
        creation_map_total(result_routing_cost, result_routing_travel_time, nodes, gdf_cycleways, gdf_footways, path,
                           options.lat, options.lon, options.mapName)
